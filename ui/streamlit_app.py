...
# Removed direct backend pipeline imports
...
def load_existing_parsed_result(
    ...
    create_langchain_documents() is reconstructed here without importing backend classes.
    """
    with (
    ...
    # =========================================================================
    # WARNING: The following loop reconstructs Document objects for the UI.
    # This logic is a duplicate of the page_text_plus_ocr branch in
    # src/parsing.py's create_langchain_documents(), and must be updated
    # manually if that method's output shape changes.
    # This is accepted tech debt for the inspector tab only, not a pattern to repeat elsewhere.
    # =========================================================================
    documents = []
    for page in pages:
    ...
    for table in tables:
        ...
        table_text = f"TABLE FOUND ON PAGE {page_number}\nTABLE INDEX: {table['table_index']}\n\nTABLE MARKDOWN:\n{table['markdown']}"
        documents.append(
            Document(
...
if selected_pdf_path is not None:
    ...
    if force_parse:
        try:
            ...
            with st.status(
                "Uploading and processing PDF via FastAPI...",
                expanded=True,
            ) as status:
                try:
                    api_base_url = os.getenv("API_BASE_URL", "http://localhost:8000")
                    with open(selected_pdf_path, "rb") as f:
                        files = {"file": (selected_pdf_path.name, f, "application/pdf")}
                    ...
                    # Write local manifest so Streamlit restore logic discovers it
                    ...
                    # Load parsed result from local JSON output dir
                    parsed = load_existing_parsed_result(
                        pdf_path=selected_pdf_path,
                        output_dir=output_dir,
                    )
                    ...
                    st.session_state.parsed_output_dir = str(output_dir)
                    st.session_state.parsed_file_fingerprint = selected_fingerprint
                    st.session_state.parsed_restore_mode = None

                    st.session_state.ingestion_result = {
                        "collection_name": upload_data["collection_name"],
                        "loaded_existing": False,
                        "source_document_count": upload_data["source_document_count"],
                        "input_document_count": upload_data["input_document_count"],
                        "indexed_document_count": upload_data["indexed_document_count"],
                        "inserted_point_count": upload_data["inserted_point_count"],
                        "file_sha256": selected_fingerprint,
                    }
                    st.session_state.active_collection = upload_data["collection_name"]
                    st.session_state.index_restore_mode = None
                    st.session_state.index_check_token = None
                    st.session_state.chat_messages = []

                    status.update(
                        label="PDF processed and indexed",
                        state="complete",
                        expanded=False,
                    )
                except Exception as exc:
...
if (
    ...
    if (
        ...
        if st.sidebar.button(
            ...
            st.session_state.chat_messages = []
            # Cached generator cleared (no-op)
            st.rerun()
